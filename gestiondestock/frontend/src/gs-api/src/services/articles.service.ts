...
class ArticlesService extends __BaseService {
  ...
  static readonly findHistoriqueCommandesClientPath = '/gestiondestock/v1/articles/historique/commandeclient/{idArticle}';
  ...
  /**
   * Rechercher la liste de tous les articles
   *
   * Cette méthode permet de rechercher la liste des articles qui existent dans la BDD)
   * @return La liste des articles a été trouvée dans la BDD
   */
   ...
   * @return successful operation
   */
  findHistoriqueCommandesClientResponse(idArticle: number): __Observable<__StrictHttpResponse<Array<LigneCommandeClientDto>>> {
    let __params = this.newParams();
    let __headers = new HttpHeaders();
    let __body: any = null;
   ...
   * @return successful operation
   */
  findHistoriqueCommandesClient(idArticle: number): __Observable<Array<LigneCommandeClientDto>> {
    return this.findHistoriqueCommandesClientResponse(idArticle).pipe(
